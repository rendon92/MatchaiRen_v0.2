...
sub printCMD() {
	...
	print STDERR "\t\t\t!!! warning: size is inclusive - i.e. 100 will be [-50,50] so 101 in length\n";
	...
	#print STDERR "\t\t-keepHeader (Keep header in output)\n";
...
while (<IN>) {
	...
	my $fpcenter = (($s+$e)/2);
	my $center = floor($fpcenter);
	if ($strand eq '-' || $strand eq '1') {
		if (abs($fpcenter-$center) > 0.25) {
			$center++;
		}
	}
	#print STDERR "center=$center\nhalf=$half\n";
