...
sub printCMD() {
	print STDERR "\n\tUsage: convertOrganismID.pl <file> <current organsism> <new organism> <output ID type> [header: yes/no] [keep original: yes/no]\n";
...
`convertIDs.pl "$tmpfile2" $org2 $idtype $header $keepFlag $keepFlag > "$tmpfile"`;
open IN, $tmpfile;
while (<IN>) {
	print $_;
}
