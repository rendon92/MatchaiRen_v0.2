...
sub printCMD {
	...
	print STDERR "\t\t\t-size given (uses the exact regions you give it, recommended to use same sized regions)\n";
	...
	print STDERR "\t\t-useNewBg (new background selection does not use preparsed genome files)\n";
	print STDERR "\t\t-genomeBg (use genome sequences for background, default)\n";
	print STDERR "\t\t-modelBg (generate background sequences by modeling sequence properties of the target sequences)\n";
	print STDERR "\t\t-bg <background BED/peak file> (genomic positions to be used as background)\n";
	print STDERR "\n";
	print STDERR "\t\t-N <#> (Number of background sequences to use for motif finding, default=100000, or 2x input)\n";
	print STDERR "\t\t-numBins <#> (number of GC bins to stratify GC-content in sequences by, default: 10)\n";
	print STDERR "\t\t-ikmer <#> (when constructing background, match kmer content of this length with targets, default: 2)\n";
	print STDERR "\t\t-pkmer <#> (when constructing background, positionally match kmer content)\n";
	print STDERR "\t\t-allowTargetOverlaps (allow selected genomic bg regions to overlap target regions)\n";
	print STDERR "\t\t-allowBgOverlaps (allow selected genomic bg regions, allow them to overlap with one another)\n";
	print STDERR "\t\t-NN <#> (When selecting genomic bg seqs, consider this many initially, default: 100000000)\n";

	print STDERR "\n\tOld Background Sequence Options:\n";
	print STDERR "\t\t-useOldBg (Use old style [4.11 and earlier] background selection, default for now)\n";
	...
	print STDERR "\n\tAdvanced options:\n";
	print STDERR "\t\t-flip (look for motifs enriched in the background instead of target sequences)\n";
	...
	print STDERR "\n";
	print STDERR "\thomer core c++ program specific options:\n";
...
sub parseCMDLine {
	...
	my $cmd = {posfile=>$ARGV[0], fg=>'', bg=>'', 
				...
				motif=>'', nomask=>0, len=>\@len, mis=>2, numSeq=>100000,rnaMode=>0,mask=>0,
				...
				flip=>0, checkOverlaps=>1,

				oldBgFlag=>1,modelFlag=>0,kmer=>2,positionFlag=>0,NN=>100000000,
				numBins=>10,numSubBins=>10,overlapTg=>0,overlapBg=>0
			};
	print STDERR "\n";
	...
	for (my $i=3;$i<@$argv;$i++) { 
		if ($ARGV[$i] eq '-bg') {
			...
			print STDERR "\n";
		} elsif ($ARGV[$i] eq '-useOldBg' ) {
			$cmd->{'oldBgFlag'} = 1;
		} elsif ($ARGV[$i] eq '-useNewBg' ) {
			$cmd->{'oldBgFlag'} = 0;
		} elsif ($ARGV[$i] eq '-allowTargetOverlaps' ) {
			$cmd->{'overlapTg'} = 1;
		} elsif ($ARGV[$i] eq '-allowBgOverlaps' ) {
			$cmd->{'overlapBg'} = 1;
			...
			$cmd->{'kmer'} = $ARGV[++$i];
			$cmd->{'positionFlag'} = 0;
		} elsif ($ARGV[$i] eq '-numBins' || $ARGV[$i] eq '-nbins') {
			$cmd->{'numBins'} = $ARGV[++$i];
		} elsif ($ARGV[$i] eq '-numSubBins' || $ARGV[$i] eq '-nsubBins') {
			$cmd->{'numSubBins'} = $ARGV[++$i];
		} elsif ($ARGV[$i] eq '-NN' ) {
			$cmd->{'NN'} = $ARGV[++$i];
		} elsif ($ARGV[$i] eq '-genomeBg' ) {
			$cmd->{'modelFlag'} = 0;
		} elsif ($ARGV[$i] eq '-modelBg' || $ARGV[$i] eq '-model') {
			$cmd->{'modelFlag'} = 1;
		} elsif ($ARGV[$i] eq '-fdr' ) {
			$cmd->{'fdr'} = $ARGV[++$i];
...
my $tmpGenomeFile = $cmd->{'output'} . "/" . $tmpID . ".genome.fa";
my $posFileSize = $cmd->{'output'} . "/target" . $cmd->{'size'} . ".pos";
my $posFileSizeBg = $cmd->{'output'} . "/bg" . $cmd->{'size'} . ".pos";
...
if ($cmd->{'oldBgFlag'}) {
	...
	if ($cmd->{'bg'} eq '') {
		...
		$bgCGPos = $preparsedDir . "/$prefix.$bestFragSize.pos";
		if ($cmd->{'gc'}==1) {
	...
	# in this case just need to extract target sequences
	if ($cmd->{'size'} ne 'given') {
		`resizePosFile.pl "$cleanPosFile" $cmd->{'size'} $cmd->{'sizeMove'} > "$posFileSize"`;
	} else {
		`cp "$cleanPosFile" "$posFileSize"`;
	...
	print STDERR "\tUsing updated background selection (v5.0+)\n";
	my $genomeFile = $cmd->{'genome'};
	if (-f $genomeFile) {
		# all good - assume this is a fasta file with the genome in it
	} else {
		# assume we're dealing with a genome directory
		if (-f $genomeDir) {
			$genomeFile = $genomeDir;
		} elsif (-d $genomeDir) {
			my $f = "$genomeDir/genome.fa";
			if (-f $f) {
				$genomeFile = $f;
			} else {
				print STDERR "\tCreating single genome FASTA:\n\t\tcat \"$genomeDir\"/*.fa > $tmpGenomeFile\n\n";
				`cat "$genomeDir"/*.fa > $tmpGenomeFile`;
				$genomeFile = $tmpGenomeFile;
				$toDelete{$tmpGenomeFile} = 1;
			}
		}
	}
	unless (-f $genomeFile) {
		print STDERR "!!! Error. Trouble establishing genome FASTA file: \"$genomeFile\"\n";
		exit;
	}
	...
	my $opt = " -gs -o \"$cmd->{'output'}/homerBg\" ";
	$opt .= " -model" if ($cmd->{'modelFlag'});
	$opt .= " -NN $cmd->{'NN'} -N $cmd->{'numSeq'}";
	$opt .= " -nbins $cmd->{'numBins'} -nsubBins $cmd->{'numSubBins'}";
	if ($cmd->{'positionFlag'}) {
		$opt .= " -pkmer $cmd->{'kmer'}";
	} else {
		$opt .= " -ikmer $cmd->{'kmer'}";
	}
	$opt .= " -allowTargetOverlaps" if ($cmd->{'overlapTg'});
	$opt .= " -allowBgOverlaps" if ($cmd->{'overlapBg'});
	if ($cmd->{'bg'} ne '') {
		if ($cmd->{'size'} ne 'given') {
			`resizePosFile.pl "$cleanBgFile" $cmd->{'size'} $cmd->{'sizeMove'} > "$posFileSizeBg"`;
		} else {
			`cp "$cleanBgFile" "$posFileSizeBg"`;
		}
		$toDelete{$posFileSizeBg}=1;
		$opt .= " -bg \"$posFileSizeBg\" ";
	}
	#print STDERR "homer2 bg -g \"$genomeFile\" -p \"$posFileSize\" $opt\n";
	open INFO, ">>$motifInfoFile";
	print INFO "homer2 bg -g \"$genomeFile\" -p \"$posFileSize\" $opt\n";
	close INFO;
	`homer2 bg -g "$genomeFile" -p "$posFileSize" $opt`;
	`mv "$cmd->{'output'}/homerBg.group.tsv" "$groupFile"`;
	`mv "$cmd->{'output'}/homerBg.seq.tsv" "$seqFile"`;

	$toDelete{$groupFile}=1;
	$toDelete{$seqFile}=1;
	unless ($cmd->{'dumpFasta'}) {
		$toDelete{"$cmd->{'output'}/homerBg.target.sequences.fasta"}=1;
		$toDelete{"$cmd->{'output'}/homerBg.bg.sequences.fasta"}=1;
	}
	$toDelete{"$cmd->{'output'}/homerBg.bg.positions.bed"}=1;
	$toDelete{"$cmd->{'output'}/homerBg.bg.stats.bed"}=1;
	$toDelete{"$cmd->{'output'}/homerBg.target.positions.bed"}=1;
	$toDelete{"$cmd->{'output'}/homerBg.target.stats.bed"}=1;
}
...
if ($cmd->{'find'} eq '') {
	...
	if ($cmd->{'oldBgFlag'}) {
		...
		if ($executeFlag == 1) {
			if ($cmd->{'bg'} ne '' || $cmd->{'local'} > 0) {
				...
				if ($cmd->{'dumpFasta'}==1) {
					`getPos.pl "$groupFile" 0 > "$tmpFile"`;
					`filterListBy.pl "$seqFile" "$tmpFile" 0 1 > "$tmpFile2"`;
					`tab2fasta.pl "$tmpFile2" > "$dumpFastaTarget"`;
					`getPos.pl "$groupFile" 1 > "$tmpFile"`;
					`filterListBy.pl "$seqFile" "$tmpFile" 0 1 > "$tmpFile2"`;
					`tab2fasta.pl "$tmpFile2" > "$dumpFastaBackground"`;
				}
				...
				my $validBgCGBins = $cmd->{'output'} . "/validBgCGBins." . $cmd->{'size'} . ".cgbins";
				if ($cmd->{'checkOverlaps'} == 1) {
					my $opt = "";
					if ($cmd->{'norevopp'} == 1) {
						$opt = ' -strand ';
					}
					`mergePeaks -cobound 1 -prefix $tmpID $opt "$bgCGPos" "$posFileSize"`;
					`filterListBy.pl "$bgCGBins" "$tmpID.coBoundBy0.txt" 0 1 > "$validBgCGBins"`;
					`rm "$tmpID.coBoundBy0.txt" "$tmpID.coBoundBy1.txt"`;
				} else {
					`cp "$bgCGBins" "$validBgCGBins"`;
				}
				...
				if ($cmd->{'cgweight'} == 1 && $cmd->{'tssweight'} == 1) { 
					...
					`cat "$targetCGBins" "$validBgCGBins" > "$tmpFile"`;
					...
					`wc -l "$targetCGBins" "$validBgCGBins"`;
					`cat "$targetCGBins" "$validBgCGBins" > "$tmpFile"`;
					#`wc -l "$tmpFile" "$targetCGBins"`;
					`makeBinaryFile.pl "$tmpFile" "$targetCGBins" > "$tmpFile2"`;
					#`wc -l "$tmpFile2" "$tmpFile"`;
				...
				`rm "$tmpFile" "$tmpFile2" "$tmpFile3" "$validBgCGBins"`;
			}
			$toDelete{$groupFile}=1;
			$toDelete{$seqFile}=1;
		}
	...
	if ($cmd->{'flip'}) {
		print STDERR "\tInverting target/background assignments\n";
		open OUT, ">$tmpFile";
		open IN, "$groupFile";
		while (<IN>) {
			chomp;
