...
`annotatePeaks.pl $peakFile $genome $tssFile $gtfFlag $gtfFile $gidFlag $annFlag $ttsFlag > $tmp`;
open IN, $tmp;
my $c = 0;
while (<IN>){ 
	$c++;
...
sub printCMD {
	...
	print STDERR "\t\t-p <peak/BED file> (regions centered on TSS to use to define proximal/distal peaks)\n";
