...
if (@ARGV < 2) {
	...
	print STDERR "\n\tOptions:\n";
	print STDERR "\t\t-test (only show commands, do not execute)\n";
...
#print STDERR "Flag='$outputFileFlag'\n";
#print STDERR "Suffix=$outputFileSuffix\n";
...
foreach(@files) {
	...
	if ($pid == 0) {
		#child process
		my $localOpt = $opt;
		if ($outputFileSuffix ne '') {
			$localOpt .= " $outputFileFlag \"$_.$outputFileSuffix\" ";
		}

		if ($suffix eq 'none') {
			print STDERR "=========\n$cmd $_ $localOpt\n";
			`$cmd "$_" $localOpt` unless ($testFlag);
		} else {
			print STDERR "=========\n$cmd $_ $localOpt > $_.$suffix\n";
			`$cmd "$_" $localOpt > $_.$suffix` unless ($testFlag);
		}
		exit(0);
	}
	push(@pids, $pid);
