...
sub printCMD {
	...
	print STDERR "\n\tIncluding premade bigWigs:\n";
	print STDERR "\t\t-bw <bigWig file1> [bigWig file2] ...\n";
...
foreach(@tagDirs) {
	...
	my $existingBigWigFlag = 0;
	if ($tagDir =~ s/^CustomBIGWIG-//) {
		$existingBigWigFlag = 1;
		my $bigWigFile = $tagDir;
		$bigWigFile =~ s/.*\///;
		my $targetfile = "$hubDir/$hubGenome/$bigWigFile";
		`cp \"$tagDir\" \"$targetfile\" `;

		my $trackName = $bigWigFile;
