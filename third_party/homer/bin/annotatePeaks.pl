...
for (my $i=2;$i<@ARGV;$i++) {
	if ($ARGV[$i] eq '-m' || $ARGV[$i] eq '-fm') {
		...
		if ($size eq 'given') {
            ...
            $size = ($sizeEnd - $sizeStart)+1;
			$masterOffset = $sizeStart;
		}
		$updateSize = 1;
...
if ($updateSize && $size ne 'given') {
	...
	`adjustPeakFile.pl "$peakFile" -size $size -move $sizeMove > "$posFile"`;
	#print STDERR "`adjustPeakFile.pl $peakFile -size $size -move $sizeMove > $posFile`;\n";
} else {
...
if ($seqFlag) {
	...
	#print STDERR "`homerTools extract $posFile $genomeDir $mflag > $seqFile`\n";
	if ($snpFile ne '') {
...
for (my $i=0;$i<@motifFiles;$i++) {
	...
	while (<IN>) {
		...
		if ($strandFlag eq '+' || $strandFlag eq '0') {
			next if ($dir eq '-' || $dir eq '1');
		} elsif ($strandFlag eq '-' || $strandFlag eq '1') {
			next if ($dir eq '+' || $dir eq '0');
		}
...
if ($histBinSize > 0) {
	...
	for (my $i=0;$i<@tagDirs;$i++) {
		...
		$offset = $masterOffset if ($masterOffset ne '');
		my $sizeRegion = $size;
	...
	for (my $i=0;$i<@covFiles;$i++) {
		...
		#print STDERR "`getPeakTags $posFile $type $bedGraphFile -peaktags $optStr > $tmpfile`\n";
	...
	if ($seqFlag) {
		...
		$offset = -$halfSize;
		$offset = $masterOffset if ($masterOffset ne '');
		`homerTools freq "$seqFile" -offset $offset $option > "$tmpfile"`;
		#print STDERR "homerTools freq $seqFile -offset -$halfSize $option > $tmpfile\n";
