...
sub printCMD {
	...
	print STDERR "\n";
	print STDERR "\thomer core c++ program execuable specific options:\n";
...
sub parseCMDLine {
	...
	my $cmd = {fg=>$ARGV[0], bg=>'', 
				...
				humanGO=>0,seqlogo=>'',ontology=>\@ont,flip=>0
				};
	print STDERR "\nSelected Options:\n";
...
if ($cmd->{'promoters'} eq 'FASTA' || $cmd->{'promoters'} eq 'fasta') {
	if ($cmd->{'fasta'} eq '') {
		if ($cmd->{'find'}) {
			...
			#print STDERR "`scrambleFasta.pl $cmd->{'fg'} > $scrambledFasta`;\n";
	...
	if ($cmd->{'noweight'} == 1) {
		...
		if ($cmd->{'rnaMode'} == 0) {
			if ($cmd->{'fasta'} eq '') {
				...
				#print STDERR "`homerTools freq $seqFile -gc $tmpCGFreq > /dev/null`;\n";
				my $col = 1;
				if ($cmd->{'gc'} == 1) {
					$col = 2;
				}
