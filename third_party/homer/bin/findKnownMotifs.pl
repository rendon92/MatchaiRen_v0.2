...
if ($homer2Flag) {
	...
	$percentBackground =~ s/\%//;
	$totalNumTargets = 1;
	$totalNumBackground = 1;
	$totalNumTargets = floor($numTargets/($percentTargets*0.01)+0.5) if ($percentTargets > 0);
	$totalNumBackground = floor($numBackground/($percentBackground*0.01)+0.5) if ($percentBackground > 0);
} else {
