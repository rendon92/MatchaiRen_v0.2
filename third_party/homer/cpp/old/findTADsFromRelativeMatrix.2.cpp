...
void scoreTADs(int argc, char** argv) {
	...
	if (tads != NULL) {
		...
		fprintf(fp,"#TAD name\tchr\tstart\tend\tstrand\tscore\tscore2\tsize");
		for (int i=0;i<numRelMatrixFiles;i++) {
		...
		for (int i=0;i<tads->numPeaks;i++) {
			...
			fprintf(fp, "%s\t%s\t%d\t%d\t%c",p->name,p->chr,p->start,p->end,dir);
			//fprintf(fp, "\t%f\t%d", p->v,p->end - p->start);
			fprintf(fp, "\t%f\t%f\t%d", p->v,p->focusRatio,p->end - p->start);
			for (int j=0;j<numRelMatrixFiles;j++) {
	...
	if (ints != NULL) {
		...
		fprintf(fp,"#interaction name\tchr1\tstart1\tend1\tchr2\tstart2\tend2\tscore\tlocalRatio");
		for (int i=0;i<numRelMatrixFiles;i++) {
			fprintf(fp,"\t%s", relMatrixFiles[i]);
		}
		fprintf(fp,"\n");
		for (int i=0;i<ints->numInteractions;i++) {
			...
			fprintf(fp, "%s\t%s\t%d\t%d\t%s\t%d\t%d",p->name,p->chr1,p->start1,p->end1,p->chr2,p->start2,p->end2);
