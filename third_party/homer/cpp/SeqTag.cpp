...
const char* HomerConfig::homeDirectory = "/home/javi/bin/homer/./";
	...
	while (str[i] != '\0') {
		if (str[i] < 48 || str[i] > 57) {
			...
			if (cigarCodes[numCodes] == 'M' || cigarCodes[numCodes] == '=') {
				...
				//if (dir == STRAND_POSITIVE && numCodes==0) {
...
void TagLibrary::optimizeTagFiles() {
	...
	for (int i=0;i<chrs->total;i++) {
		...
		ct->optimizeTagFile();
		if (mCflag) {
			if (parseAlignmentCpGMinValue > 0) {
				ct->removeTagsByLength(parseAlignmentCpGMinValue,INT_MAX);
			}
		}
		totalTags += ct->totalTags;
		//fprintf(stderr, "\t%s\t%lf\t%d\t%lf\n",chrKeys[i],ct->totalTags,ct->totalPositions,totalTags);;
		totalPositions += ct->totalPositions;
		gsizeEstimate += ct->appearentSize;
...
void ChrTags::removeTagsByLength(int minLen, int maxLen) {
	optimizeOverride = 1;
	readTagFile();
	
	double newTotalTags = 0;
	int last=0;
	for (int i=0;i<totalPositions;i++) {
		if (tags[i].len >= minLen && tags[i].len <= maxLen) {
			if (i==last) {
			} else {
				tags[last].copy(&(tags[i]));
			}
			newTotalTags += tags[last].v;
			last++;
		}
	}
	totalPositions = last;
