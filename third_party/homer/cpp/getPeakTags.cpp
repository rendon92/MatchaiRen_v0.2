...
int main(int argc, char** argv) {
	...
	for (int i=startIndex;i<argc;i++) {
		if (argv[i][0] == '-') {
			...
			} else if (strcmp(argv[i],"-petags")==0) {
				//doesn't actually do anything
				outputMode = OUTPUT_MODE_PETAGS;
	...
	if (vcfFile != NULL) {
		...
		if (tagAutoCorrelation) {
			...
			} else if (outputMode == OUTPUT_MODE_PEAKTAGS || outputMode==OUTPUT_MODE_TAGS || outputMode==OUTPUT_MODE_PETAGS) {
				peaks->addTagLibrary(tags);
