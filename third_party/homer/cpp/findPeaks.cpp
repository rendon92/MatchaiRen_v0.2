	...
	for (int i=1;i<argc;i++) {
		...
		if (argv[i][0] == '-') {
			if (strcmp(argv[i],"-i")==0) {
				...
				sscanf(argv[++i], "%lf",&f);
				pf->endFold = f;
			} else if (strcmp(argv[i],"-tsrFold")==0 || strcmp(argv[i],"-tssFold")==0) {
				double f = 3.0;
				sscanf(argv[++i], "%lf",&f);
			...
			} else if (strcmp(argv[i],"-tsrSize")==0 || strcmp(argv[i],"-tssSize")==0) {
				...
				if (strcmp(argv[i],"factor")==0) {
					...
					//pf->clonalFold = 0.0;
					superFlag = 1;
				} else if (strcmp(argv[i],"atac")==0) {
					pf->style = PEAK_STYLE_HISTONE;
					pf->stitchMode = REGION_MODE_HISTONE;
					pf->regionFlag = 1;
					pf->localFold = 2;
					pf->regionSubDivision = 8;
					pf->setPeakSize(50);
					pf->minDist = 50;
					pf->poisson = 0.001;
					pf->filterMode = PEAKFINDER_FILTER_MODE_POISSON;
					...
					fragmentLength=1;
					fragmentLengthInput=1;
				} else if (strcmp(argv[i],"tsr")==0 || strcmp(argv[i],"tss")==0) {
	...
	if (outputfile != NULL) {
		if (strcmp(outputfile,"auto") == 0) {
			...
			if (pf->style == PEAK_STYLE_HISTONE) {
				...
				sprintf(outputfile,"%s/tsr.txt",directory);
...
void printCMD() {
	...
	fprintf(stderr, "\t\t\tatac (ATAC-seq, uses -region -size 50 -minDist 50 -regionRes 8 -L 2, output: regions.txt)\n"); 
	...
	fprintf(stderr, "\t\t\ttsr (TSR -transcription start region- identification from 5' RNA sequencing, tsr.txt)\n"); 
	...
	fprintf(stderr, "\t\t-tsrSize <#> (size of region for initiation detection/artifact size, default: %d)\n",GROSEQ_TSSSIZE); 
	...
	fprintf(stderr, "\t\t-tsrFold <#> (fold enrichment for new initiation dectection, default: %.1lf)\n",GROSEQ_TSSFOLDCHANGE); 
